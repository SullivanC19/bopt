...
NodeDataManager_Cover::NodeDataManager_Cover(
        ...
        tids_error_callback,
        supports_error_lb_class_callback)
                   {}
...
bool NodeDataManager_Cover::updateData(Node *best, Error upperBound, Attribute attribute, Node *left, Node *right, float splitPenalty, Itemset itemset) {
    auto *freq_best = (CoverNodeData*)best->data, *freq_left = (CoverNodeData*)left->data, *freq_right = (CoverNodeData*)right->data;
    Error error = freq_left->error + freq_right->error + splitPenalty;
...
NodeData *NodeDataManager_Cover::initData(RCover *cov, Depth currentMaxDepth, int hashcode) {
    ...
    if (tids_error_class_callback == nullptr && tids_error_callback == nullptr) {
        //python fast error
        if (supports_error_class_callback != nullptr || supports_error_lb_class_callback != nullptr) {
            bool lb = supports_error_lb_class_callback != nullptr;
            function<vector<float>(RCover *)> callback = lb ? *supports_error_lb_class_callback : *supports_error_class_callback;
            ...
            if (lb) {
                data->lowerBound = infos[2];
                // cout << "Error: " << error << " maxclass: " << maxclass << " lower bound: " << data->lowerBound << endl;
            }
            // else {
            //     cout << "Error: " << error << " maxclass: " << maxclass << endl;
            // }
        }
        //default error
        else {
